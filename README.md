...
- **检索评测**：45 组真实查询 × 6 策略（含语义改写与纯视觉查询），指标含 Recall@k / MRR / NDCG；
...
| E1 +关键词门控(v1) | 0.733 | 0.900 | **0.956** | 0.851 | 0.855 |
| E2 +向量门控(v2) | 0.733 | 0.900 | **0.956** | 0.851 | 0.855 |
...
- **工程测试**：116 个 pytest 用例（LLM/Embedding 全部 mock，离线可跑），GitHub Actions 自动运行。
...
- **门控多模态融合**：朴素三路融合在评测中拉低 Recall@1（-0.167），据此改为仅视觉查询启用 VL 向量，Recall@5 升至 0.956；门控本身也随评测演进：v1 关键词表 → v2 向量质心相似度（阈值 LOO 校准、查询向量复用召回结果零额外调用、库外探针泛化 57% → 100%），v1 降级为向量不可用时的兜底；
