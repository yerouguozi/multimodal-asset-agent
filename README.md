...
- **素材级混合检索**：自实现 BM25 + bge-m3 文本向量 + 图片 VL 向量，RRF 融合 + bge-reranker 精排；VL 向量按查询意图门控启用（向量质心判定 + 留一法校准阈值，向量不可用自动回退关键词规则）；支持以图搜图与音视频转写片段时间戳检索；
...
- **可观测与治理**：检索全链路日志（API / Agent 工具），平均与 P95 延迟、来源与策略分布、高频查询；门控决策逐条落库（判定方式 / margin / 是否视觉），指标页展示判定分布；每用户模型调用按日/月/小时配额限流，成本按真实模型调用记录（降级模式零记账）；
- **向量后端可切换**：本地 npz / Milvus（素材与片段各自独立命名空间，连接失败自动降级）。

## 🧪 评测体系

- **检索评测**：51 组真实查询 × 6 策略（含语义改写、纯视觉与跨语言查询），指标含 Recall@k / MRR / NDCG，报告附 Bootstrap 95% 置信区间；报告与前端评测页数据由 `backend/scripts/eval_retrieval.py` 一键生成；

| 策略 | Recall@1 | Recall@3 | Recall@5 | MRR | NDCG@3 |
|---|---|---|---|---|---|
| A 纯 BM25（基线） | 0.539 | 0.598 | 0.598 | 0.588 | 0.589 |
| B +文本向量 RRF | 0.765 | 0.853 | 0.922 | 0.848 | 0.835 |
| D +朴素三路融合(VL) | 0.539 | 0.726 | 0.765 | 0.678 | 0.668 |
| E1 +关键词门控(v1) | **0.765** | 0.912 | **0.961** | **0.868** | **0.872** |
| E2 +向量门控(v2) | 0.726 | 0.912 | **0.961** | 0.849 | 0.857 |
| C +重排精排 | **0.814** | **0.951** | 0.951 | 0.905 | 0.912 |

- **门控专项**：14 条库外探针（中文新词 / 英文视觉 / 中英任务意图，与种子集零重叠）测门控泛化——v1 关键词规则 8/14，v2 向量质心 14/14；阈值由种子留一法（LOO）自动校准，详见 `docs/eval-reports/门控设计.md`；
- **跨语言发现**：v2 门控对英文视觉查询判型全对（v1 完全漏判），但 VL 跨语言文本→图对齐弱于 bge-m3 文本→描述，E2 在跨语言查询上 Recall@1 略低于 E1（Recall@3/@5 持平）——「判对意图」≠「VL 有收益」，v3 方向是按收益门控（gate-on-benefit），生产默认 full 策略由重排兜底；
- **Agent 质量评测**：`backend/scripts/eval_agent.py` 9 个用例量化「防幻觉两道防线」——工具规划命中率、引用合法率、库外反例如实说空、忠实度（LLM 裁判）与延迟，报告输出至 `docs/eval-reports/Agent质量评测.md`；
- **片段级对比**：`backend/scripts/eval_passage_vs_asset.py` 离线样例对比整篇 Top-5 与片段 Top-1（原文含答案短语判定），报告输出至 `docs/eval-reports/`；
- **工程测试**：118 个 pytest 用例（LLM/Embedding 全部 mock，离线可跑），GitHub Actions 自动运行；另有 nightly 评测工作流每天用真实模型重跑检索评测并自动提交报告（需在仓库 Secrets 配置 `SILICONFLOW_API_KEY`）。
...
graph TD
    ...
    CHUNK --> CV[(片段向量库)]
    PROC --> VS[(素材向量库)]
    API --> SEARCH[意图门控 + BM25<br/>+ 向量 RRF + 重排]
...
设计原则：工具结果是事实来源，LLM 只负责规划与组织语言；检索按查询意图门控启用多模态信号（向量质心判定），避免“信号越多越好”的直觉误区。
...
- **门控多模态融合**：朴素三路融合在评测中拉低 Recall@1（-0.226），据此改为仅视觉查询启用 VL 向量，Recall@5 升至 0.961；门控本身也随评测演进：v1 关键词表 → v2 向量质心相似度（阈值 LOO 校准、查询向量复用召回结果零额外调用、库外探针泛化 57% → 100%），v1 降级为向量不可用时的兜底；跨语言查询暴露 v3 方向——按收益门控（gate-on-benefit）；
...
- 前端 E2E 测试；
- 超过 600 秒的长音频分片转录与合并；
- 门控升级：从检索日志挖掘误判说法扩充种子，数据量足够后演进为轻量意图分类器。
