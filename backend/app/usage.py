...
def record_usage(asset_id: int | None, model: str, operation: str) -> None:
    """独立开事务记录一次模型调用（处理器/工具里没有现成 session 时使用）。"""
    if not model:
        return
    with SessionLocal() as db:
        db.add(UsageLog(asset_id=asset_id, model=model, operation=operation, cost_estimate=estimate_cost(model)))
        db.commit()


def usage_summary(db) -> dict:
    rows = db.query(UsageLog).order_by(UsageLog.id.desc()).all()
